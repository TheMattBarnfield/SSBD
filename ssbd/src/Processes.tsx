...
const Processes: React.SFC<{}> = props => {
  ...
  const image = <img src={Trees} alt='Processes' style={{width:'100%'}}/>

  return <ContentPage title='Products and Processes'>
    <div style={{padding:12}}>
      <Grid container spacing={24}>
        <Grid container alignContent='flex-start' xs={12} md={5} spacing={8}>
          <Grid item xs={6} md={12} style={{padding:24}}>{preamble}</Grid>
          <Grid item xs={6} md={12} style={{padding:24}}>{image}</Grid>
        </Grid>
